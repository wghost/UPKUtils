...
std::string ModParser::GetText()
{
    ...
    if (isKey)
    {
        ...
        while ( modFile.good() )
        {
            ...
            {
                modFile.clear(modFile.rdstate() & ~std::ios::eofbit); /// to parse last line correctly
                break;
            }
            str += std::string("\n") + line;
        }
...
int ModParser::FindNext()
{
    ...
    modFile.clear(modFile.rdstate() & ~std::ios::eofbit); /// to get last line value correctly
    modFile.seekg(savedPos, std::ios::beg);
    isKey = (keyIdx != -1);
